...
void pwm_init()
{
    ...
    gpio_set_function(20, GPIO_FUNC_PWM);//Middle Left  (ML) Motor PWM
    gpio_set_function(21, GPIO_FUNC_PWM);//Middle Right (MR) Motro PWM
...
void set_duty_rl(float duty)
{
    ...
    if (duty>DUTYMAX)duty=DUTYMAX;
    if (duty<DUTYMIN)duty=DUTYMIN;
    pwm_set_chan_level(Slice_num_middle, PWM_CHAN_B, duty);
    //printf("%4.0f ", duty);
}
...
void set_duty_fl(float duty)
{
    ...
    if (duty>DUTYMAX)duty=DUTYMAX;
    if (duty<DUTYMIN)duty=DUTYMIN;
    pwm_set_chan_level(Slice_num_middle, PWM_CHAN_A, duty);
    //printf("%4.0f ", duty);
}
...
void payload_half(void)
{
    pwm_set_chan_level(Slice_num_servo, PWM_CHAN_A, SERVO_HALF);
    // printf("release\n");
}